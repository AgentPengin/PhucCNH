...
int main() {
	...
	cin >> k >> l >> m >> n >> d;
	for (int i = k;i<=d;i += k) check[i] = true;
	for (int i = l;i<=d;i += l) check[i] = true;
	for (int i = m;i<=d;i += m) check[i] = true;
	for (int i = n;i<=d;i += n) check[i] = true;
	for (int i = 1;i<=d;i++) if (check[i] == true) res++;
		cout << res;
 	return 0;
}
// Code cua Phuc ;-;
